// Система зашифрованных звонков - ИСПРАВЛЕННАЯ ВЕРСИЯ
class EncryptedCallSystem {
    ...
    // УЛУЧШЕННЫЙ ОБРАБОТЧИК ONTRACK ДЛЯ МОБИЛЬНЫХ УСТРОЙСТВ
    setupTrackHandler(withVideo) {
        return (event) => {
    ...
    async startCall(targetUser, withVideo = true) {
        try {
            ...
            // УЛУЧШЕННЫЙ ОБРАБОТЧИК получения удаленного потока
            this.peerConnection.ontrack = this.setupTrackHandler(withVideo);
